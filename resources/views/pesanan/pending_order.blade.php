        ...
        <div class="col-md-9 ms-sm-auto col-lg-12 px-md-12">
         ...
         <div class=" d-flex justify-content-between align-items-center flex-wrap">
          ...
          <div class="col-sm-12 me-auto">
            <form action="/market-pending-order/{{ $slug_id }}" method="GET">
              <div class="col-sm-4">
                     ...
                     <div class="d-flex text-center">
                      ...
                      <div class="d-flex flex-column w-100">
                          ...
                          <div class="d-flex mt-4">
                           @if ($orders->status_pesanan != 'cancel')
                              @if ($orders->pesanan_diantar == true)
                              <form action="/pesanan/{{ $orders->kode_pesanan }}" method="POST" class="me-auto" id="pending{{ $orders->id }}">
                                @csrf
                                @method('put')
                                <input type="hidden" name="pending" value="{{ hash('sha256' , 'pending-data-cant-be-forget') }}" class="d-none">
                                <button type="button" class="btn univ_btn" id="proses{{ $orders->id }}">Proses Sepatu</button>
                              </form>
                              @else
                              <button type="button" class="btn btn-danger me-auto">Proses Sepatu</button>
                              @endif
                            @else
                            <form action="/pesanan/{{ $orders->kode_pesanan }}" method="POST" class="me-auto">
                              @csrf
                              @method('delete')
                              <input type="hidden" name="cancelOrder" value="{{ hash('sha256' , 'this-is-canceled-order') }}" class="d-none">
